...
export default function InitialValue({ VariableComponent: Variable, classes, title, type, options, name, value, onChange, ...props }) {
          ...
          >
            {_get(options, type).map(({ code, description: desc, refValue }) => (
              <MenuItem key={code} value={code}>
                <Tooltip title={(<pre>{refValue ? JSON.stringify(refValue, null, 2) : 'Null'}</pre>)}>
                  <ListItemText {...desc} />
                </Tooltip>
              </MenuItem>
            ))}
                ...
                <ListItemSecondaryAction>
                  <Tooltip title={dt('btn-add-property')}>
                    <IconButton
                      ...
                      color="primary"
                      onClick={() => {
                        const property = { uid: uuid(), type: 'input', description: `Variable_${Math.floor(Math.random() * 10000)}` };
